#!/usr/bin/env python
# -*- coding: utf-8 -*-
# Created by HazzaCheng on 2020-05-20
...
class NasCoauthorcs(nn.Module):
    def __init__(self, features_num, num_class, num_layers=2, dropout=0.5, hidden=64, edge_num=1000, **kwargs):
        ...
        for _ in range(num_layers):
            cell = NasCoauthorcsCell(his_dim, cur_dim, hidden_dim, output_dim, multi_head)
...
class NasCoauthorcsCell(nn.Module):
    # best structure:{'action': [0, 'linear', 1, 'sage', 'tanh', 'concat'], 'hyper_param': [0.005, 0.5, 1e-05, 64]}
    def __init__(self, his_dim, cur_dim, hidden_dim, output_dim, multi_head):
        super(NasCoauthorcsCell, self).__init__()
    ...
    def forward(self, h, x, edge_index, edge_weight):
        ...
        o3 = F.tanh(torch.cat([o1, o2], dim=1))
        return his, o3
